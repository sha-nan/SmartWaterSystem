...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagOxygenMotor_OnOff)
    {
        info->event[info->num] = EVENT_OxygenMotor_OnOff;
        info->num++;
        dataPoints->valueOxygenMotor_OnOff = gizStandardDecompressionValue(OxygenMotor_OnOff_BYTEOFFSET,OxygenMotor_OnOff_BITOFFSET,OxygenMotor_OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagWaterMotor_OnOff)
    {
        info->event[info->num] = EVENT_WaterMotor_OnOff;
        info->num++;
        dataPoints->valueWaterMotor_OnOff = gizStandardDecompressionValue(WaterMotor_OnOff_BYTEOFFSET,WaterMotor_OnOff_BITOFFSET,WaterMotor_OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagsolenoid_valve)
    {
        info->event[info->num] = EVENT_solenoid_valve;
        info->num++;
        dataPoints->valuesolenoid_valve = gizStandardDecompressionValue(solenoid_valve_BYTEOFFSET,solenoid_valve_BITOFFSET,solenoid_valve_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSolenoidValve_Water)
    {
        info->event[info->num] = EVENT_SolenoidValve_Water;
        info->num++;
        dataPoints->valueSolenoidValve_Water = gizStandardDecompressionValue(SolenoidValve_Water_BYTEOFFSET,SolenoidValve_Water_BITOFFSET,SolenoidValve_Water_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSolenoidValve_Soil)
    {
        info->event[info->num] = EVENT_SolenoidValve_Soil;
        info->num++;
        dataPoints->valueSolenoidValve_Soil = gizStandardDecompressionValue(SolenoidValve_Soil_BYTEOFFSET,SolenoidValve_Soil_BITOFFSET,SolenoidValve_Soil_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueOxygenMotor_OnOff != cur->valueOxygenMotor_OnOff)
    {
        GIZWITS_LOG("valueOxygenMotor_OnOff Changed\n");
        ret = 1;
    }
    if(last->valueWaterMotor_OnOff != cur->valueWaterMotor_OnOff)
    {
        GIZWITS_LOG("valueWaterMotor_OnOff Changed\n");
        ret = 1;
    }
    if(last->valuesolenoid_valve != cur->valuesolenoid_valve)
    {
        GIZWITS_LOG("valuesolenoid_valve Changed\n");
        ret = 1;
    }
    if(last->valueSolenoidValve_Water != cur->valueSolenoidValve_Water)
    {
        GIZWITS_LOG("valueSolenoidValve_Water Changed\n");
        ret = 1;
    }
    if(last->valueSolenoidValve_Soil != cur->valueSolenoidValve_Soil)
    {
        GIZWITS_LOG("valueSolenoidValve_Soil Changed\n");
        ret = 1;
    }
    if(last->valueLiquidLevel_Low != cur->valueLiquidLevel_Low)
    {
        GIZWITS_LOG("valueLiquidLevel_Low Changed\n");
        ret = 1;
    }
    if(last->valueLiquidLevel_High != cur->valueLiquidLevel_High)
    {
        GIZWITS_LOG("valueLiquidLevel_High Changed\n");
        ret = 1;
    }

    if(last->valueSoilHumidity != cur->valueSoilHumidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSoilHumidity Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(OxygenMotor_OnOff_BYTEOFFSET,OxygenMotor_OnOff_BITOFFSET,OxygenMotor_OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueOxygenMotor_OnOff);
    gizStandardCompressValue(WaterMotor_OnOff_BYTEOFFSET,WaterMotor_OnOff_BITOFFSET,WaterMotor_OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWaterMotor_OnOff);
    gizStandardCompressValue(solenoid_valve_BYTEOFFSET,solenoid_valve_BITOFFSET,solenoid_valve_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesolenoid_valve);
    gizStandardCompressValue(SolenoidValve_Water_BYTEOFFSET,SolenoidValve_Water_BITOFFSET,SolenoidValve_Water_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSolenoidValve_Water);
    gizStandardCompressValue(SolenoidValve_Soil_BYTEOFFSET,SolenoidValve_Soil_BITOFFSET,SolenoidValve_Soil_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSolenoidValve_Soil);
    gizStandardCompressValue(LiquidLevel_Low_BYTEOFFSET,LiquidLevel_Low_BITOFFSET,LiquidLevel_Low_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLiquidLevel_Low);
    gizStandardCompressValue(LiquidLevel_High_BYTEOFFSET,LiquidLevel_High_BITOFFSET,LiquidLevel_High_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLiquidLevel_High);
    ...
    devStatusPtr->valueSoilHumidity = gizY2X(SoilHumidity_RATIO,  SoilHumidity_ADDITION, dataPoints->valueSoilHumidity); 
...
* @return none
*/
void gizwitsInit(void)//gizwits协议相关初始化接口
{    
