...
void usmart_reset_runtime(void)
{
	...
	TIM_SetAutoreload(TIM5,0XFFFF);//将重装载值设置到最大
	TIM_SetCounter(TIM5,0);		//清空定时器的CNT
...
{
	if(TIM_GetFlagStatus(TIM5,TIM_FLAG_Update)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=TIM_GetCounter(TIM5);
...
{ 		    		  			    
	if(TIM_GetITStatus(TIM5,TIM_IT_Update)==SET)//溢出中断
	{
		...
		TIM_SetCounter(TIM5,0);		//清空定时器的CNT
		TIM_SetAutoreload(TIM5,100);//恢复原来的设置		    				   				     	    	
	}				   
